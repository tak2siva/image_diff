...
def templateMatch(templateStr, actual):
    origImg = cv2.imread(actual)
    img_rgb = cv2.imread(actual)
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
    template = cv2.imread(templateStr,0)
    ...
    res2 = origImg[top_left[1]:top_left[1]+h, top_left[0]:top_left[0]+w]
    # cv2.imshow('res2',res2)
    # cv2.waitKey(0)

    return [cv2.imread(templateStr), res2]